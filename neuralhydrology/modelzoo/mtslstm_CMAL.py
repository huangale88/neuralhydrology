...
class MTSLSTMCMAL(BaseModel):
    ...
    def _calculate_cmal_mean(self, mu: torch.Tensor, b: torch.Tensor, tau: torch.Tensor, pi: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
        """Calculates the mean of the CMAL mixture distribution for tied_frequency"""
        ...
        mean_mixture = torch.sum(pi_safe * component_means, dim=-1, keepdim=True) # Keep the last dimension for consistency
        return mean_mixture
    ...
    def forward(self, data: dict[str, torch.Tensor | dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        ...
        for idx, freq in enumerate(self._frequencies):
            ...
            if isinstance(self.heads[freq], nn.Module) and self.heads[freq].__class__.__name__ == 'CMAL':
                ...
                outputs[f'y_hat_{freq}'] = mean_prediction
